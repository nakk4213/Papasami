  ...
  return (
    <div className="min-h-screen bg-[#050714]">
      <aside className="fixed inset-y-0 left-0 hidden w-72 border-r border-white/10 bg-black/20 p-5 backdrop-blur-xl lg:block">
        ...
        <Link href="/" className="mt-5 flex items-center gap-3 rounded-xl border border-white/10 px-3 py-2 text-sm text-muted-foreground transition hover:bg-white/10 hover:text-white">
          <Home className="size-4" />
          Back to homepage
        </Link>
        <nav className="mt-8 grid gap-2">
          {links[role].map(([label, href, Icon]) => (
            <Link key={href} href={href} className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm text-muted-foreground transition hover:bg-white/10 hover:text-white">
      ...
      <div className="lg:pl-72">
        <header className="sticky top-0 z-40 flex h-16 items-center justify-between border-b border-white/10 bg-[#050714]/80 px-4 backdrop-blur-xl sm:px-6">
          <div>
